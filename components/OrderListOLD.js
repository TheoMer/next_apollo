...
const OrderList = props => {
  ...
  const { me, user_ip, user_Agent, url, urlReferer, data: { orders, loading: loadingQuery, error: errorQuery, startPolling, stopPolling, subscribeToMore }} = props;
  stopPolling(600);
  ...
  if (errorQuery) return <Error erorr={errorQuery} />;
  if (orders.length >= 1) {
    usersOrders =  (
      <div>
...
export default memo(userOrdersQuery(OrderList));
